  ...
  for (let n = 0; n < arr.length; n++) {
    if (arr[n].idMeal == e.target.id) {
      if (!fav.includes(arr[n])) {
        fav.push(arr[n]);
        renderingListToADD();
        alert("Meal successfully added into FAVOURITE list");
        return;
      }
      else {
        if (e.target.dataset.id != "1") {
          alert("This Meal already into your FAV list")
          return;
        }
      }
    }
